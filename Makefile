...
userprogs: $(USERPROG_ELF) $(BUILD_DIR)/userprogs/input.elf
...
disk: userprogs
	...
	$(PYTHON) tools/create_disk.py $(DISK_IMG) $(USERPROG_ELF) $(BUILD_DIR)/userprogs/input.elf
...
$(BUILD_DIR)/userprogs/%.elf: $(USERPROG_DIR)/%.c $(USERPROG_DIR)/api.h $(USERPROG_DIR)/userprog.ld include/graphics_abi.h include/input_abi.h
	@mkdir -p $(BUILD_DIR)/userprogs
